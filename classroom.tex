...
please adopt a Code of Conduct like the one in \appref{s:conduct}
...
\gref{g:co-teaching}{Co-teaching} describes any situation
...
All of these models create more opportunities for lateral knowledge transfer than teaching alone.
...
because of the \hreffoot{https://en.wikipedia.org/wiki/Dunning\%E2\%80\%93Kruger\_effect}{Dunning-Kruger effect}:
...
\appref{s:preassess} presents a short pre-assessment questionnaire
...
\begin{aside}{False Beginners}
  ...
  They may be indistinguishable from \gref{g:absolute-beginner}{absolute beginners} on pre-assessment tests,
...
\gref{g:pair-programming}{Pair programming} is a software development practice
...
even though taking notes on a computer is generally less effective
than taking notes using pen and paper~\cite{Muel2014}:
...
Students often find that taking notes together is distracting
the first time they try it
because they have to split their attention between
what the instructor is saying
and what their peers are writing (\secref{s:load-split-attention}).
...
You can also use sticky notes as \gref{g:minute-cards}{minute cards}.
Before each break,
...
The one-up/one-down technique described in \seclbl{s:classroom-practices}
...
but is extraneous load to them (\chapref{s:load}).
...
Using the questionnaire in \appref{s:preassess} as a template,
