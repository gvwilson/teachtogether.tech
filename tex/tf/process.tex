\chaplbl{A Lesson Design Process}{s:process} % FIXME
...
The personas in \secref{s:intro-audience} have the five points listed above,
...
shared with other teachers (\secref{s:process-maintainability}).
...
(\secref{s:community-governance} and
\secref{s:joining-contributing}).
...
on HTML and CSS in \secref{s:process-objectives} and classify each
