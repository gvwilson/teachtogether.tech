\chaplbl{Teaching Online}{s:online} % FIXME
...
up their individual misconceptions (\chapref{s:models}): if they
...
\appref{s:meetings} for some tips on how to make these
...
part in online discussions (\secref{s:classroom-coc}).
...
\begin{aside}{Points for Improvement}
  ...
  lecture (\secref{s:classroom-notetaking}); in online courses, you
...
(\secref{s:performance-live}). \cite{Chen2009} offers useful tips
...
of a concept map (\secref{s:memory-concept-maps}).
...
(\secref{s:classroom-peer}), and has been studied intensively over
...
even further. And as discussed in \secref{s:individual-peer},
...
\begin{aside}{Trust, but Educate}
  ...
  (\secref{s:individual-peer}) can be equally effective. Before
...
\begin{aside}{Hybrid Presence}
  ...
  (\secref{s:joining-using}). This scales well, saves on travel
...
your answers with the checklist in \secref{s:checklists-teacheval}.)
