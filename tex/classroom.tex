...
\chapref{s:intro} said that every workshop should have and enforce a
Code of Conduct like the one in \appref{s:conduct}.  If you are a
...
\begin{callout}{Helping}
  ...
  keep an eye on the shared notes (\secref{s:classroom-notetaking})
...
\item
  ...
  (\secref{s:classroom-notetaking}), keep an eye on the students to
...
\appref{s:preassess} presents a short preassessment questionnaire
...
form of real-time elaboration (\secref{s:individual-strategies}): it
...
please heed the advice in \secref{s:classroom-innovate} and stick to
...
We frequently use sticky notes as \glossref{g:minute-cards}{minute
  cards}: before each break, learners take a minute to write one
...
is extraneous load to them (\chapref{s:load}).
...
(\secref{s:classroom-mixed}).  In-floor power outlets so that you
...
and how much they actually learn (\secref{s:pck-baseline}).  Instead,
