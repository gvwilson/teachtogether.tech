...
\begin{callout}{Not Just for Students}
  ...
  powerful reason to co-teach (\secref{s:classroom-together}): just as
...
``usefulness once mastered'' (\figref{f:motivation-what}).
...
\item[Using the J word (``just'') or feigning surprise] (i.e., saying
  ...
  heard of Y?''). As discussed in \chapref{s:memory}, this signals to
...
\secref{s:motivation-inclusivity}, and doing so improves outcomes for
...
under-represented groups: as discussed in \secref{s:pck-baseline},
...
as noted in \secref{s:performance-live}, your typos show your class
...
We said in \secref{s:intro-code-of-conduct} that classes should
enforce a Code of Conduct like the one in \appref{s:conduct}.  This is
...
As discussed in \secref{s:classroom-enforce}, the details of the Code
...
direction.  We return to this in \chapref{s:community}.
...
preparatory privilege (\secref{s:classroom-mixed}) may not be any one
...
tech; we will return to this topic in \chapref{s:community}.
