...
clear up their individual misconceptions (\chapref{s:models}): if they
...
\item[Have learners contribute to collective knowledge,] e.g., take
  notes together (\secref{s:classroom-notetaking}), serve as classroom
...
\item[Encourage or require learners to do some of their work in small
  ...
  \appref{s:meetings} for some tips on how to make these discussions
  fair and productive.)
...
\item[Create, publicize, and enforce a code of conduct] so that
  ...
  online discussions (\secref{s:intro-code-of-conduct}).
...
\item[Identify and clear up misconceptions early]
  (\chapref{s:models}). If data shows that learners are struggling
...
\begin{callout}{Points for Improvement}
  ...
  them to do this as you lecture (\secref{s:classroom-notetaking}); in
...
As mentioned in \chapref{s:performance}, a teaching technique called
...
(\secref{s:performance-live}).  \cite{Chen2009} offers useful tips for
creating and critiquing screencasts and other videos;
\figref{f:online-screencasting} reproduces the patterns that paper
presents and the relationships between them, and is also a good
example of a concept map (\secref{s:memory-concept-maps}).
...
(\secref{s:classroom-peer}), and has been studied intensively over the
...
\secref{s:individual-peer}, collusion and bias are \emph{not}
...
\begin{callout}{Trust, but Educate}
  ...
  review (\secref{s:individual-peer}) can be equally effective.
...
the benefits discussed in \secref{s:performance-live}, and when
combined with collaborative note-taking
(\secref{s:classroom-notetaking}) it can come pretty close to
...
\begin{callout}{Hybrid Presence}
  ...
  streaming video (\secref{s:joining-using}).  This scales well, saves
...
can compare your answers with the checklist in \appref{s:teacheval}.)

\exercise{Classifying Online Coding Lessons}{individual}{15}
