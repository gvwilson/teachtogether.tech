\chaplbl{Cognitive Load}{s:load} % FIXME
...
(\secref{s:classroom-practices}). It also encourages learners to
...
\begin{aside}{Efficiency vs. Extent}
  ...
  \chapref{s:pck}, deconstructing code by tracing it or debugging it
...
in \secref{s:individual-strategies}), researchers' perspectives may
...
(\secref{s:memory-seven-plus-or-minus}) and giving each chunk a name helps
...
opportunity for self-explanation (\secref{s:individual-strategies}).

\seclbl{Split Attention}{s:load-split-attention}
...
discussing dual coding in \secref{s:individual-strategies}).
...
\seclbl{Minimal Manuals}{s:load-minimal}
...
\seclbl{Other Models of Learning}{s:load-theory}
...
\seclbl{Exercises}{s:load-exercises}
...
Spot'' in \secref{s:memory-exercises} will give you an idea of how
